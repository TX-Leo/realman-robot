...
void execute(const control_msgs::FollowJointTrajectoryGoalConstPtr& goal, Server* as) {
    ...
    if  (point_num > 3) //判断当moveit规划的路点数大于3时为有效规划并进行三次样条插值
    {
        ...
        /*
        ROS_INFO("****************************************start*************************************************");
        ROS_INFO("The point number before cubicSpline is:%d",point_num);
        
        for (int i = 0; i < point_num; i++)
        {
        	ROS_INFO("The position before cubicSpline:[%f, %f, %f, %f, %f, %f]",p_joint1[i], p_joint2[i], p_joint3[i], p_joint4[i], p_joint5[i], p_joint6[i]);
			ROS_INFO("The velocity before cubicSpline:[%f, %f, %f, %f, %f, %f]",v_joint1[i], v_joint2[i], v_joint3[i], v_joint4[i], v_joint5[i], v_joint6[i]);
			ROS_INFO("The accelerated velocity before cubicSpline:[%f, %f, %f, %f, %f, %f]",a_joint1[i], a_joint2[i], a_joint3[i], a_joint4[i], a_joint5[i], a_joint6[i]);
        }
		ROS_INFO("*****************************************end**************************************************");
        */
        // 实例化样条
                    ...
                    if (spline.loadData(time_from_start, p_joint4, point_num, 0, 0, cubicSpline::BoundType_First_Derivative))
                    {
                        ...
                        if (spline.loadData(time_from_start, p_joint5, point_num, 0, 0, cubicSpline::BoundType_First_Derivative))
                        {
                            ...
                            if (spline.loadData(time_from_start, p_joint6, point_num, 0, 0, cubicSpline::BoundType_First_Derivative))
                            {
                                ...
                                /*
                                ROS_INFO("****************************************start*************************************************");
        						ROS_INFO("The point number after cubicSpline is:%d",p_joint1_.size());
								for (int i = 0; i < p_joint1_.size(); i++)
								{
									ROS_INFO("The position after cubicSpline:[%f, %f, %f, %f, %f, %f]",p_joint1_[i], p_joint2_[i], p_joint3_[i], p_joint4_[i], p_joint5_[i], p_joint6_[i]);
									ROS_INFO("The velocity after cubicSpline:[%f, %f, %f, %f, %f, %f]",v_joint1_[i], v_joint2_[i], v_joint3_[i], v_joint4_[i], v_joint5_[i], v_joint6_[i]);
									ROS_INFO("The accelerated velocity after cubicSpline:[%f, %f, %f, %f, %f, %f]",a_joint1_[i], a_joint2_[i], a_joint3_[i], a_joint4_[i], a_joint5_[i], a_joint6_[i]);
								}
								ROS_INFO("*****************************************end**************************************************");
								*/
...
int main(int argc, char** argv)
{
    ...
    target_pub = nh.advertise<dual_arm_msgs::JointPos>("JointPos", 300);
    pub_getArmStateTimerSwitch = nh.advertise<std_msgs::Bool>("GetArmStateTimerSwitch", 200);
    pub_getArmJoint = nh.advertise<std_msgs::Empty>("GetArmJoint_Cmd", 100);
    
    // 定义一个服务器
    Server server(nh, "follow_joint_trajectory", boost::bind(&execute, _1, &server), false);
    // 服务器开始运行
    server.start();
    ros::spin();
