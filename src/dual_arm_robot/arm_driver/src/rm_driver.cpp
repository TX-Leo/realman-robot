...
void SetHandPosture_Callback(const dual_arm_msgs::Hand_Posture msg)
{
...
void SetHandSeq_Callback(const dual_arm_msgs::Hand_Seq msg)
{
...
void SetHandSpeed_Callback(const dual_arm_msgs::Hand_Speed msg)
{
...
void SetHandForce_Callback(const dual_arm_msgs::Hand_Force msg)
{
...
void SetHandAngle_Callback(const dual_arm_msgs::Hand_Angle msg)
{
    int res = 0;
    int i = 0;
...
void SetJointStep_Callback(const dual_arm_msgs::Joint_Step msg)
{
    int res = 0;
    uint8_t num;
    float angle;
...
void LiftHeightCtr_Callback(const dual_arm_msgs::Lift_Height msg)
{
...
void LiftSpeedCtr_Callback(const dual_arm_msgs::Lift_Speed msg)
{
...
void MoveJ_Callback(const dual_arm_msgs::MoveJ msg)
{
    int res = 0;
    int i = 0;
    byte speed;
...
void MoveL_Callback(const dual_arm_msgs::MoveL msg)
{
    int res = 0;
    byte speed;
    POSE target;
...
void MoveC_Callback(const dual_arm_msgs::MoveC msg)
{
    int res = 0;
    byte speed;
...
void JointTeach_Callback(const dual_arm_msgs::Joint_Teach msg)
{
...
void PosTeach_Callback(const dual_arm_msgs::Pos_Teach msg)
{
...
void OrtTeach_Callback(const dual_arm_msgs::Ort_Teach msg)
{
...
void StopTeach_Callback(const dual_arm_msgs::Stop_Teach msg)
{
 ...
 * *********************************************************************************/
void MoveJ_P_Callback(const dual_arm_msgs::MoveJ_P msg)
{
    int res = 0;
    byte speed;
    POSE target;
...
void ChangeToolName_Callback(const dual_arm_msgs::ChangeTool_Name msg)
{
    int res = 0;
...
void ChangeWorkFrame_Callback(const dual_arm_msgs::ChangeWorkFrame_Name msg)
{
    int res = 0;
...
void GetArmState_Callback(const dual_arm_msgs::GetArmState_Command msg)
{
    int res = 0;
 ...
 * *********************************************************************************/
//开始复合模式拖动示教
void StartMultiDragTeach_Callback(const dual_arm_msgs::Start_Multi_Drag_Teach msg)
{
...
//力位混合控制
void SetForcePosition_Callback(const dual_arm_msgs::Set_Force_Position msg)
{
    int res = 0;
    if ((msg.mode > 0) && (msg.mode < 4))
    {
...
//位姿透传力位混合补偿
void ForcePositionMovePose_Callback(const dual_arm_msgs::Force_Position_Move_Pose msg)
{
    int res = 0;
    POSE target;
    target = Quater_To_Euler(msg.Pose);
...
//角度透传力位混合补偿
void ForcePositionMoveJiont_Callback(const dual_arm_msgs::Force_Position_Move_Joint msg)
{
    int res = 0;
    float joint[6];
...
void ManualSetForcePose_Callback(const dual_arm_msgs::Manual_Set_Force_Pose msg)
{
...
void JointPos_Callback(const dual_arm_msgs::JointPos msg)
{
    int res = 0;
    int i = 0;
    float joint[6];
...
void Arm_DO_Callback(const dual_arm_msgs::Arm_Digital_Output msg)
{
    int res = 0;
...
void Arm_AO_Callback(const dual_arm_msgs::Arm_Analog_Output msg)
{
    int res = 0;
    if ((msg.num > 0) && (msg.num < 5))
    {
...
void Tool_DO_Callback(const dual_arm_msgs::Tool_Digital_Output msg)
{
    int res = 0;
...
void Tool_AO_Callback(const dual_arm_msgs::Tool_Analog_Output msg)
{
    int res = 0;
...
void Gripper_Pick_Callback(dual_arm_msgs::Gripper_Pick msg)
{
    int res = 0;
...
void Gripper_Pick_On_Callback(dual_arm_msgs::Gripper_Pick msg)
{
    int res = 0;
...
void Gripper_Set_Callback(dual_arm_msgs::Gripper_Set msg)
{
    int res = 0;
...
void Stop_Callback(const dual_arm_msgs::Stop msg)
{
...
void Joint_Enable_Callback(const dual_arm_msgs::Joint_Enable msg)
{
    int res = 0;
...
void IO_Update_Callback(const dual_arm_msgs::IO_Update msg)
{
    int res = 0;
    // Arm IO Update
    if (msg.type == 1)
 ...
 * *********************************************************************************/
void TurtleCtr_Callback(const dual_arm_msgs::Turtle_Driver msg)
{
...
void Movep_Fd_Callback(const dual_arm_msgs::CartePos msg)
{
...
int main(int argc, char **argv)
{
    ...
    // msg
    dual_arm_msgs::Arm_IO_State Arm_IO;
    dual_arm_msgs::Tool_IO_State Tool_IO;
    dual_arm_msgs::Plan_State Plan;
    dual_arm_msgs::ChangeTool_State ChangeTool_State;
    dual_arm_msgs::ChangeWorkFrame_State ChangeWorkFrame_State;
    dual_arm_msgs::Arm_Current_State Arm_State;
    dual_arm_msgs::Force_Position_State Force_Position_State;
    dual_arm_msgs::Six_Force Six_Force;
    std_msgs::Bool state;
    dual_arm_msgs::Joint_Current joint_current;
    dual_arm_msgs::ArmState armState;
    dual_arm_msgs::LiftState liftState;
    ...
    MoveJ_Cmd = nh_.subscribe("MoveJ_Cmd", 10, MoveJ_Callback);
    MoveL_Cmd = nh_.subscribe("MoveL_Cmd", 10, MoveL_Callback);
    MoveC_Cmd = nh_.subscribe("MoveC_Cmd", 10, MoveC_Callback);
    JointPos_Cmd = nh_.subscribe("JointPos", 10, JointPos_Callback);
    Arm_DO_Cmd = nh_.subscribe("Arm_Digital_Output", 10, Arm_DO_Callback);
    Arm_AO_Cmd = nh_.subscribe("Arm_Analog_Output", 10, Arm_AO_Callback);
    Tool_DO_Cmd = nh_.subscribe("Tool_Digital_Output", 10, Tool_DO_Callback);
    Tool_AO_Cmd = nh_.subscribe("Tool_Analog_Output", 10, Tool_AO_Callback);
    Gripper_Cmd = nh_.subscribe("Gripper_Pick", 10, Gripper_Pick_Callback);
    Gripper_Set_Cmd = nh_.subscribe("Gripper_Set", 10, Gripper_Set_Callback);
    Emergency_Stop = nh_.subscribe("Emergency_Stop", 1, Stop_Callback);
    Joint_En = nh_.subscribe("Joint_Enable", 10, Joint_Enable_Callback);
    IO_Update = nh_.subscribe("IO_Update", 1, IO_Update_Callback);

    sub_getArmStateTimerSwitch = nh_.subscribe("GetArmStateTimerSwitch", 1, getArmStateTimerSwitch_Callback);

    ros::Subscriber MoveP_Fd_Cmd = nh_.subscribe("MoveP_Fd_Cmd", 10, Movep_Fd_Callback);

    sub_getCurrJointCurrent = nh_.subscribe("GetCurrentJointCurrent", 10, GetCurrJointCurrent_Callback);
    sub_setJointStep = nh_.subscribe("SetJointStep", 10, SetJointStep_Callback);
    sub_getTotalWorkFrame = nh_.subscribe("GetTotalWorkFrame", 10, GetTotalWorkFrame_Callback);
    sub_setArmPower = nh_.subscribe("SetArmPower", 10, SetArmPower_Callback);
    sub_setToolVoltage = nh_.subscribe("SetToolVoltage", 10, SetToolVoltage_Callback);

    sub_setHandPosture = nh_.subscribe("Hand_SetPosture", 10, SetHandPosture_Callback);
    sub_setHandSeq = nh_.subscribe("Hand_SetSeq", 10, SetHandSeq_Callback);
    sub_setHandAngle = nh_.subscribe("Hand_SetAngle", 10, SetHandAngle_Callback);
    sub_setHandSpeed = nh_.subscribe("Hand_SetSpeed", 10, SetHandSpeed_Callback);
    sub_setHandForce = nh_.subscribe("Hand_SetForce", 10, SetHandForce_Callback);
     ...
     * *********************************************************************************/
    MoveJ_P_Cmd = nh_.subscribe("MoveJ_P_Cmd", 10, MoveJ_P_Callback);
    Sub_ChangeToolName = nh_.subscribe("ChangeToolName_Cmd", 10, ChangeToolName_Callback);
    Sub_ChangeWorkFrame = nh_.subscribe("ChangeWorkFrame_Cmd", 10, ChangeWorkFrame_Callback);
    Sub_GetArmState = nh_.subscribe("GetArmState_Cmd", 10, GetArmState_Callback);
    sub_getCurrArmState = nh_.subscribe("GetCurrentArmState", 10, GetCurrentArmState_Callback);

    // publisher
    ChangeTool_Name = nh_.advertise<dual_arm_msgs::ChangeTool_State>("ChangeTool_State", 1);
    ChangeWorkFrame_Name = nh_.advertise<dual_arm_msgs::ChangeWorkFrame_State>("ChangeWorkFrame_State", 1);
    ArmCurrentState = nh_.advertise<dual_arm_msgs::Arm_Current_State>("Arm_Current_State", 10);
    pub_armCurrentState = nh_.advertise<dual_arm_msgs::ArmState>("ArmCurrentState", 10);
     ...
     * 20220808修改: 增加对机械臂的复合拖动示教、力位混合控制、结束力位混合控制
     开启透传力位混合控制补偿模式、透传力位混合补偿（角度）、透传力位混合补偿（位姿）、关闭透传力位混合控制补偿模式
     * *********************************************************************************/
    Sub_StartMultiDragTeach = nh_.subscribe("StartMultiDragTeach_Cmd", 10, StartMultiDragTeach_Callback);
    Sub_StopDragTeach = nh_.subscribe("StopDragTeach_Cmd", 10, StopDragTeach_Callback);
    Sub_SetForcePosition = nh_.subscribe("SetForcePosition_Cmd", 10, SetForcePosition_Callback);
    Sub_StopForcePostion = nh_.subscribe("StopForcePostion_Cmd", 10, StopForcePostion_Callback);

    Sub_ToGetSixForce = nh_.subscribe("GetSixForce_Cmd", 10, GetSixForce_Callback);
    Sub_ClearForceData = nh_.subscribe("ClearForceData_Cmd", 10, ClearForceData_Callback);
    Sub_SetForceSensor = nh_.subscribe("SetForceSensor_Cmd", 10, SetForceSensor_Callback);
    Sub_ManualSetForcePose = nh_.subscribe("ManualSetForcePose_Cmd", 10, ManualSetForcePose_Callback);
    Sub_StopSetForceSensor = nh_.subscribe("StopSetForceSensor_Cmd", 10, StopSetForceSensor_Callback);

    Sub_GetArmJoint = nh_.subscribe("GetArmJoint_Cmd", 100, GetArmJoint_Callback);

    sub_lift_setHeight = nh_.subscribe("Lift_SetHeight", 10, LiftHeightCtr_Callback);
    sub_setLiftSpeed = nh_.subscribe("Lift_SetSpeed", 10, LiftSpeedCtr_Callback);
    sub_getLiftState = nh_.subscribe("Lift_GetState", 10, GetLiftState_Callback);

    nh_.setCallbackQueue(&queue_forcePositionMove);
    Sub_StartForcePositionMove = nh_.subscribe("StartForcePositionMove_Cmd", 10, StartForcePositionMove_Callback);
    Sub_StopForcePositionMove = nh_.subscribe("StopForcePositionMove_Cmd", 10, StopForcePositionMove_Callback);
    Sub_ForcePositionMovePose = nh_.subscribe("ForcePositionMovePose_Cmd", 10, ForcePositionMovePose_Callback);
    Sub_ForcePositionMoveJiont = nh_.subscribe("ForcePositionMoveJiont_Cmd", 10, ForcePositionMoveJiont_Callback);
     ...
     * *********************************************************************************/
    sub_setJointTeach = nh_.subscribe("Arm_JointTeach", 10, JointTeach_Callback);
    sub_setPosTeach = nh_.subscribe("Arm_PosTeach", 10, PosTeach_Callback);
    sub_setOrtTeach = nh_.subscribe("Arm_OrtTeach", 10, OrtTeach_Callback);
    sub_setStopTeach = nh_.subscribe("Arm_StopTeach", 10, StopTeach_Callback);
    /***** ********************************END****************************************/
    ...
    // publisher
    pub_StartMultiDragTeach_result = nh_.advertise<std_msgs::Bool>("StartMultiDragTeach_result", 10);
    pub_StopDragTeach_result = nh_.advertise<std_msgs::Bool>("StopDragTeach_result", 10);
    pub_SetForcePosition_result = nh_.advertise<std_msgs::Bool>("SetForcePosition_result", 10);
    pub_StopForcePostion_result = nh_.advertise<std_msgs::Bool>("StopForcePostion_result", 10);

    pub_GetSixForce = nh_.advertise<dual_arm_msgs::Six_Force>("GetSixForce", 100);
    pub_ClearForceData_result = nh_.advertise<std_msgs::Bool>("ClearForceData_result", 10);
    pub_ForceSensorSet_result = nh_.advertise<std_msgs::Bool>("ForceSensorSet_result", 10);
    pub_StopSetForceSensor_result = nh_.advertise<std_msgs::Bool>("StopSetForceSensor_result", 10);

    pub_StartForcePositionMove_result = nh_.advertise<std_msgs::Bool>("StartForcePositionMove_result", 10);
    pub_StopForcePositionMove_result = nh_.advertise<std_msgs::Bool>("StopForcePositionMove_result", 10);
    pub_Force_Position_State = nh_.advertise<dual_arm_msgs::Force_Position_State>("Force_Position_State", 10);
    pub_Force_Position_Move_result = nh_.advertise<std_msgs::Bool>("Force_Position_Move_result", 10);
    /***** ********************************END****************************************/

    // publisher
    Joint_State = nh_.advertise<sensor_msgs::JointState>("/joint_states", 300);
    Arm_IO_State = nh_.advertise<dual_arm_msgs::Arm_IO_State>("Arm_IO_State", 1);
    Tool_IO_State = nh_.advertise<dual_arm_msgs::Tool_IO_State>("Tool_IO_State", 1);
    Plan_State = nh_.advertise<dual_arm_msgs::Plan_State>("Plan_State", 1);
    pub_PoseState = nh_.advertise<geometry_msgs::Pose>("Pose_State", 1);
    pub_currentJointCurrent = nh_.advertise<dual_arm_msgs::Joint_Current>("Joint_Current", 1);
    pub_liftState = nh_.advertise<dual_arm_msgs::LiftState>("LiftState", 1);
    pub_setGripperResult = nh_.advertise<std_msgs::Bool>("Set_Gripper_Result", 1);
    ...
    /***** ********************************START***************************************
     * 20230411修改: 从launch文件中读取arm_joints数组,启动多个该节点(如双臂)时匹配具体的关节名称
     * *********************************************************************************/
    std::vector<std::string> arm_joints;
    if (pnh.getParam("arm_joints", arm_joints))
    {
