...
def pred_decode(end_points):
    ...
    for i in range(batch_size):
        grasp_center = end_points['xyz_graspable'][i].float()# 1024,3

        grasp_score = end_points['grasp_score_pred'][i].float()# 1024,12,4
        grasp_score = grasp_score.view(M_POINT, NUM_ANGLE*NUM_DEPTH)# 1024,48
        grasp_score, grasp_score_inds = torch.max(grasp_score, -1)# 1024  # [M_POINT]
        grasp_score = grasp_score.view(-1, 1)#1024,1
        grasp_angle = (grasp_score_inds // NUM_DEPTH) * np.pi / 12# 1024
        grasp_depth = (grasp_score_inds % NUM_DEPTH + 1) * 0.01#1024
        grasp_depth = grasp_depth.view(-1, 1) #1024,1
        grasp_width = 1.2 * end_points['grasp_width_pred'][i] / 10.#1024,12,4
        grasp_width = grasp_width.view(M_POINT, NUM_ANGLE*NUM_DEPTH)#1024,48
        grasp_width = torch.gather(grasp_width, 1, grasp_score_inds.view(-1, 1))#1024,1
        grasp_width = torch.clamp(grasp_width, min=0., max=GRASP_MAX_WIDTH)#1024,1

        approaching = -end_points['grasp_top_view_xyz'][i].float()#1024,3
        grasp_rot = batch_viewpoint_params_to_matrix(approaching, grasp_angle)#1024,3,3
        grasp_rot = grasp_rot.view(M_POINT, 9)#1024,9

        # merge preds
        grasp_height = 0.02 * torch.ones_like(grasp_score)
        obj_ids = -1 * torch.ones_like(grasp_score)
        grasp_preds.append( #1024,17
