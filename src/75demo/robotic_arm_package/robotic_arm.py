...
class Get_Joint():
    ...
    def Get_Joint_Speed(self, retry=5):
        """
    ...
    def Get_Joint_Acc(self, retry=5):
    ...
    def Get_Joint_Min_Pos(self, retry=5):
    ...
    def Get_Joint_Max_Pos(self, retry=5):
    ...
    def Get_Joint_EN_State(self, retry=5):
        """
    ...
    def Get_Joint_Err_Flag(self, retry=5):
        """
...
class Tcp_Config():
    ...
    def Get_Arm_Line_Speed(self, retry=5):
        """
    ...
    def Get_Arm_Line_Acc(self, retry=5):
        """
    ...
    def Get_Arm_Angular_Speed(self, retry=5):
        """
    ...
    def Get_Arm_Angular_Acc(self, retry=5):
        """
    ...
    def Get_Collision_Stage(self, retry=5):
        """
...
class Tool_Frame():
    ...
    def Get_Current_Tool_Frame(self, retry=5):
        """
    ...
    def Get_Given_Tool_Frame(self, name, retry=5):
        """
    ...
    def Get_All_Tool_Frame(self, retry=5):
...
class Work_Frame():
    ...
    def Get_Current_Work_Frame(self, retry=5):
        """
    ...
    def Get_Given_Work_Frame(self, name, retry=5):
        """
    ...
    def Get_All_Work_Frame(self, retry=5):
        """
...
class Arm_State():
    def Get_Current_Arm_State(self, retry=5):
    ...
    def Get_Arm_All_State(self, retry=5):
        """
    ...
    def Get_Arm_Plan_Num(self, retry=5):
...
class Move_Plan():
    def Movej_Cmd(self, joint, v, r=0, block=True):
        """
       Movej_Cmd 关节空间运动
       ArmSocket socket句柄
       joint 目标关节1~7角度数组
    ...
    def Movel_Cmd(self, pose, v, r=0, block=True):
        """
        笛卡尔空间直线运动
       ...
       return:0-成功，失败返回:错误码, rm_define.h查询
        ...
        self.pDll.Movel_Cmd.argtypes = (ctypes.c_int, Pose, ctypes.c_byte, ctypes.c_float, ctypes.c_bool)
    ...
    def Movec_Cmd(self, pose_via, pose_to, v, r=0, loop=0, block=True):
        """
        Movec_Cmd 笛卡尔空间圆弧运动
        ...
        tag = self.pDll.Movec_Cmd(self.nSocket, pose1, pose2, v, r, loop, block)
    ...
    def MoveRotate_Cmd(self, rotateAxis, rotateAngle, choose_axis, v, r=0, block=True):
        ...
        tag = self.pDll.MoveRotate_Cmd(self.nSocket, rotateAxis, rotateAngle, pose, v, r, block)

        logger_.info(f'MoveRotate_Cmd:{tag}')

        return tag

    def MoveCartesianTool_Cmd(self, joint_cur, movelengthx, movelengthy, movelengthz, m_dev, v, r=0, block=True):
        """
    ...
    def Movej_P_Cmd(self, pose, v, r=0, block=True):
        """
        该函数用于关节空间运动到目标位姿
        ...
        self.pDll.Movej_P_Cmd.argtypes = (
            ctypes.c_int, Pose, ctypes.c_byte, ctypes.c_float, ctypes.c_bool)
...
class Set_controller():
    ...
    def Get_Controller_State(self, retry=5):
        """
    ...
    def Get_Arm_Power_State(self, retry=5):
        """
    ...
    def Get_Arm_Software_Version(self, retry=5):
        """
    ...
    def Get_System_Runtime(self, retry=5):
        """
    ...
    def Get_High_Ethernet(self, retry=5):
    ...
    def Get_Wired_Net(self, retry=5):
        """
...
class Drag_Teach():
    ...
    def Start_Multi_Drag_Teach(self, mode, block=True):
        """
        ...
        self.pDll.Start_Multi_Drag_Teach.argtypes = (ctypes.c_int, ctypes.c_int, ctypes.c_bool)
        self.pDll.Start_Multi_Drag_Teach.restype = self.check_error

        tag = self.pDll.Start_Multi_Drag_Teach(self.nSocket, mode, block)
...
class Six_Force():
    ...
    def Stop_Set_Force_Sensor(self, block=True):
        ...
        """
        Stop_Set_Force_Sensor 在标定六维力过程中，如果发生意外，发送该指令，停止机械臂运动，退出标定流程
        :param block:RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
        :return:0-成功，失败返回:错误码, rm_define.h查询.
...
class one_force():
    def Get_Fz(self):
        ...
        tag = self.pDll.Get_Fz(self.nSocket, ctypes.byref(fz), ctypes.byref(zero_fz), ctypes.byref(work_fz), ctypes.byref(tool_fz))
...
class ModbusRTU():
    ...
    def Read_Multiple_Holding_Registers(self, port, address, num, device):
        ...
        self.pDll.Read_Multiple_Holding_Registers.argtypes = (
            ctypes.c_int, ctypes.c_byte, ctypes.c_int, ctypes.c_byte, ctypes.c_int, ctypes.c_int8 * num)
        self.pDll.Read_Multiple_Holding_Registers.restype = self.check_error

        coils_data = (ctypes.c_int * num)()

        tag = self.pDll.Read_Multiple_Holding_Registers(self.nSocket, port, address, num, device)
    ...
    def Get_Read_Multiple_Coils(self, port, address, num, device):
        ...
        self.pDll.Get_Read_Multiple_Coils.argtypes = (
            ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int8 * num)
        self.pDll.Get_Read_Multiple_Coils.restype = self.check_error

        coils_data = (ctypes.c_int * num)()

        tag = self.pDll.Get_Read_Multiple_Coils(self.nSocket, port, address, num, device)
...
class Set_Lift():
    ...
    def Get_Lift_State(self, retry=5):
        """
...
class Expand():
    ...
    def Expand_Get_State(self, retry=5):
        """
    ...
    def Expand_Get_Config(self, retry=5):
        """
...
class UDP():
    def Get_Realtime_Push(self, retry=5):
        """
...
class Algo:
    ...
    def Algo_Inverse_Kinematics(cls, q_in, q_pose, flag):
        ...
        tag = cls.pDll.Algo_Inverse_Kinematics(q_in, po1, q_out, flag)
...
class Program_list():
    ...
    def Get_Program_Run_State(self, cycle_num):
        """
        ...
        self.pDll.Get_Program_Run_State.argtypes = [ctypes.c_int, ctypes.POINTER(ctypes.c_int),
                                                  ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int),
                                                  ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int)]
        ...
        id = ctypes.c_int()
        plan_num = ctypes.c_int()
        loop_num = (ctypes.c_int*cycle_num)()
        loop_cont = (ctypes.c_int*cycle_num)()

        result = self.pDll.Get_Program_Run_State(self.nSocket, ctypes.byref(run_state), ctypes.byref(id),
                                               ctypes.byref(plan_num), loop_num, loop_cont)

        return result, run_state.value, id.value, plan_num.value, list(loop_num), list(loop_cont)
...
class Arm(Set_Joint, Get_Joint, Tcp_Config, Tool_Frame, Work_Frame, Arm_State, Initial_Pose, Move_Plan, Teaching,
          ...
          ModbusRTU, Set_Lift, Force_Position, Algo, Online_programming, Expand, UDP, Program_list):
    ...
    def API_Version(self):
        ...
        logger_.info(f'API_Version:{api_name}')
        time.sleep(0.5)
